...
def create_colormap(shape):
    vert = shape.vert.detach().cpu().numpy()
    minx = np.min(vert[:, 0])
    maxx = np.max(vert[:, 0])
    miny = np.min(vert[:, 1])
    maxy = np.max(vert[:, 1])
    minz = np.min(vert[:, 2])
    maxz = np.max(vert[:, 2])
    colors = np.ones((vert.shape[0], 3))
    colors[:, 0] = (vert[:, 0]-minx) / (maxx-minx)
    colors[:, 1] = (vert[:, 1]-miny) / (maxy-miny)
    colors[:, 2] = (vert[:, 2]-minz) / (maxz-minz)
    return colors

def plot_shape_matching(shape_x, shape_y, cm1, cm2):
    vert_x = shape_x.vert.detach().cpu().numpy()
    vert_y = shape_y.vert.detach().cpu().numpy()
    vert_x = vert_x - vert_x.mean(0, keepdims=True)
    vert_y = vert_y - vert_y.mean(0, keepdims=True)
    ...
    fig = plt.figure()
    ax = fig.add_subplot(121, projection='3d')
    ax.plot_trisurf(vert_x[:, 0], vert_x[:, 1], vert_x[:, 2], triangles=shape_x.get_triv_np(), cmap=cm1, linewidths=0.1)
    plt.title("X")
    ax.set_xlim(-max_bound, max_bound)
    ax.set_ylim(-max_bound, max_bound)
    ax.set_zlim(-max_bound, max_bound)
    ax = fig.add_subplot(122, projection='3d')
    ax.plot_trisurf(vert_y[:, 0], vert_y[:, 1], vert_y[:, 2], triangles=shape_y.get_triv_np(), cmap=cm2, linewidths=0.1)
    plt.title("Y")
    ax.set_xlim(-max_bound, max_bound)
    ax.set_ylim(-max_bound, max_bound)
    ax.set_zlim(-max_bound, max_bound)
...
class Shape:
    ...
    def subsample_random_lnd(self, n_vert, lnds):
        tensor = [0] * (n_vert + len(lnds))
        for i in range(len(tensor)):
            if i < n_vert:
                tensor[i] = random.randint(0, self.vert.shape[0]-1)
            else:
                tensor[i] = lnds[i-n_vert]
        self.samples = my_long_tensor(tensor)

    def subsample_lnd_nn(self, lnds, n=30):
        tensor = [0] * (len(lnds)*n)
        dist = np.sqrt(distance_matrix(self.vert[lnds], self.vert))
        sort_dist = np.argsort(dist, axis=1)
        for i in range(len(lnds)):
            tensor[n*i:n*i+n] = sort_dist[i][:n]
        self.samples = my_long_tensor(tensor)

    def subsample_lnd(self, lnds):
        self.samples = my_long_tensor([lnds[i] for i in range(len(lnds))])
